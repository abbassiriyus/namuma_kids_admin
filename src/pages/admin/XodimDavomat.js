...
export default function XodimDavomat() {
  ...
  const getBackgroundColorAndLateness = (startTime, endTime, startTime_plan, endTime_Plan) => {
    const result = { color: '#FFFFFF', sticker: '😊', lateness: '-' };
    ...
    if (endTime && endTime_Plan) {
      try {
        ...
        if (earlyMinutes < 0) {
          ...
          result.sticker = totalDeviation <= 10 ? '😐' : totalDeviation <= 60 ? '😔' : '😣';
                                  ...
                                  <>
                                    <div>
                                      {vaqtInfo.start_time ? (
                                        ...
                                        >
                                          {loadingStates[`kelgan_${xodim.id}`] ? 'Saqlanmoqda...' : '✅ Ishga keldim'}
                                        </button>
                                      )}
                                    </div>
                                    <div>
                                      {vaqtInfo.end_time ? (
                                        ...
                                        >
                                          {loadingStates[`ketgan_${xodim.id}`] ? 'Saqlanmoqda...' : '🚪 Ishdan ketdim'}
                                        </button>
                                      )}
                                    </div>
                                  </>
                                ) : (
                                  workedHours || (kunDavomat ? <span className={styles.icon}>❌</span> : '')
                                )}
                              </td>
                            );
                          })}
